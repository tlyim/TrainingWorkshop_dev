...
- Useful even for developing slides and writing documents in Quarto/Markdown

- To reject an inline suggestion, 
  - indicate _Leave me alone for now_ by pressing `Esc`  


# Plan vs. Agent mode {.smaller}

- **Plan mode**: You describe a problem for the AI assistant to
  formulate a plan to solve the problem, which you *can review and agree
  on **before implementation***
  - The AI assistant is not allowed to change any code under this mode
- **Agent mode**: The AI assistant can autonomously
  generate/alter/delete code as needed to achieve the goal specified by
  you (or in the agreed plan)
  - Depending on the configuration, the AI assistant may still need your
    permissions on taking actions that may have significant
    consequences,
    - e.g., using certain tools such as **bash** commands, accessing the
...
- The **context window** size determines the amount of information that
  can be considered at once by an AI model

- The invisible **system prompt** used to guide the model's behavior and
  responses can already take up a significant portion of the context
  window,

  - leaving less space for user input (**prompts**) and model output
    (**responses**)

- A larger context window allows the model to understand and generate
  more ***coherent and contextually relevant responses***, especially
  for complex tasks that require understanding of long documents or
  conversations.

- Models with smaller context windows handle context exhaustion through
  **compression/compaction** (i.e., truncating or summarizing the
  input),

  - which can lead to loss of important input information and less
    accurate output
...
- At least ***1M tokens*** context window (e.g., *DeepSeek V4.1 Flash*
  on *OpenCode **Go***) is necessary for a series of related complex
  tasks emerging from a long conversation

- ***At least 200K tokens*** context window (e.g., *MiMo V2.5 Free* on
  *OpenCode **Zen***) is needed for serious work other than simple tasks

- If ***two rounds*** of prompt-and-response aren't getting you closer
  to your goal (e.g., fixing a bug),

  - consider switching to a more capable (likely more expensive) model
    but ***more expensive models are not necessarily better***

- Experiment with different models to find out which ones work well for
...
# OpenCode / GitHub Copilot / Codex / Claude Code

- Among them, switching backend LLMs via third-party APIs is fully supported by **OpenCode** and GitHub Copilot
  - only Codex **CLI** has similar support
  - no support for the Codex web app

- The support for third-party APIs is in the blood of **OpenCode** as a community-driven open-source project 
  - So far, OpenCode **Zen** has a few free models with generous **rate limits** and context window sizes (e.g., *MiMo V2.5 Free* with 200K tokens context window) for users to get started without paying anything.
  - OpenCode **Go** offers a low-cost monthly subscription plan with a range of serious models (e.g., *DeepSeek V4.1 Flash* with 1M tokens context window) suitable for more complex tasks.
